<div>
    {{-- Header --}}
    <x-page-header :title="'Edit Order #' . $order->id" subtitle="Update customer, items, and totals. Stock changes are applied safely.">
        <x-slot name="pretitle">
            <div class="flex items-center gap-3">
                <a href="{{ route('orders.view', $order->id) }}" class="text-blue-600 hover:text-blue-700 font-medium text-sm">← Back to Order</a>
            </div>
        </x-slot>
        <x-slot name="actions">
            <div class="text-right">
                <p class="text-sm text-gray-600">Current Total</p>
                <p class="text-2xl font-bold text-blue-600">₱{{ number_format($this->cartTotal) }}</p>
                <p class="text-xs text-gray-500">Existing payments stay the same; payment status recalculates after update.</p>
            </div>
        </x-slot>
    </x-page-header>
            ...
            @if($customer_id)
                <div class="md:col-span-2">
                    <div class="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <button wire:click="$set('customer_id', '')" class="bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded-md transition" title="Change customer">
            ...
            @if($customer_id)
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" wire:model="vehicle_type" placeholder="e.g., SUV" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-lg shadow p-6 sticky top-24 max-h-[calc(100vh-150px)] overflow-y-auto">
                        ...
                        @foreach($cart as $key => $item)
                            <div class="border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition">
                                        ...
                                        <p class="text-xs text-gray-600">
                                            @if($item['type'] === 'product')
                                                ...
                                                @if(isset($item['available_stock']))
                                                    <span class="text-gray-500">Available: {{ $item['available_stock'] }}</span>
                                                @endif
                                            @else
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <input type="number" wire:change="updateQuantity('{{ $key }}', $event.target.value)" value="{{ $item['quantity'] }}" min="1" class="w-20 px-2 py-1 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    <div class="text-right">
                ...
                <div class="border-t pt-4 space-y-2 mb-4">
                    ...
                    <div class="flex justify-between py-2 border-t">
                        <span class="font-semibold text-gray-900">New Total</span>
                ...
                @if($customer_id && count($cart) > 0)
                    <button wire:click="save" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg transition">
                ...
                <p class="text-xs text-gray-600 mt-3 text-center">
                    Stock is adjusted by differences; payment status recalculates automatically.
                </p>
            </div>
        </div>
    </div>
