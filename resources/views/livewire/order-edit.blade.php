<div>
    {{-- Header --}}
    <x-page-header title="Edit Order #{{ $orderId }}" subtitle="Update order details, products, services, and expenses">
        <x-slot name="actions">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <h3 class="font-medium text-gray-900 mb-3">Customer Details</h3>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($branches as $branch)
                            <button type="button"
                                ...
                                class="px-3 py-2 rounded-md border font-medium text-sm transition
                                    {{ $branch_id == $branch->id ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-blue-50' }}">
                                {{ $branch->name }}
                            </button>
                        @endforeach
                    </div>
                            ...
                            @foreach($products as $product)
                                ...
                                <button 
                                    ...
                                    <p class="text-xs {{ $isDifferentBranch ? 'text-red-600 font-semibold' : 'text-gray-500' }} pt-2 border-t border-gray-200">
                                        <span class="font-semibold">Branch:</span> {{ $branches->firstWhere('id', $product->branch_id)?->name ?? 'Unknown' }}
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-4">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                ...
                                @if($discount_value > 0)
                                    <p class="text-xs text-gray-600 mt-1">Current: <span class="font-bold text-red-600">₱{{ number_format($discount_value) }}</span></p>
                                @endif
                            </div>
                            <button 
                                ...
                                <div class="space-y-2">
                                    ...
                                    @if($discount_value > 0)
                                        <div class="bg-green-50 p-3 rounded border border-green-200">
                                            <p class="text-xs text-gray-600">Discount Amount: <span class="font-bold text-green-600">₱{{ number_format($discounted_amount) }}</span></p>
                                        </div>
                                    @endif
